...
static void ctx_update_status(struct ctx_t *ctx, enum ctx_status_enum status)
{
	...
	/* Update status */
	ctx->status = status_diff; //Changed here
	...
	if (ctx->status & ctx_running)
		ke_list_insert_tail(ke_list_running, ctx);
	if (ctx->status & ctx_zombie)
		ke_list_insert_tail(ke_list_zombie, ctx);
	if (ctx->status & ctx_finished)
		ke_list_insert_tail(ke_list_finished, ctx);
	if (ctx->status & ctx_suspended)
		ke_list_insert_tail(ke_list_suspended, ctx);
	if (ctx->status & ctx_alloc)
		ke_list_insert_tail(ke_list_alloc, ctx);
