...
void push_interrupt (interrupt itrp) {
	schedint[++sched_count] = itrp;
	int pos;
	for (pos = sched_count; pos>1 && schedint[pos].instno < schedint[pos/2].instno; pos /=2) {
		schedint[pos] = schedint[pos/2];
		schedint[pos/2] = itrp;
	}
}

interrupt pop_interrupt() {
	interrupt ret = schedint[1];
	int pos, nextpos;
	sched_count--;
	if (sched_count > 0) {
		for (pos=1; pos <= sched_count/2; pos=nextpos) {
			if (schedint[2*pos].instno < schedint[2*pos+1].instno)
				nextpos = 2*pos;
			else
				nextpos = 2*pos + 1;
			
			if (schedint[sched_count+1].instno > schedint[nextpos].instno)
				schedint[pos] = schedint[nextpos];
			else break;
		}
		schedint[pos] = schedint[sched_count+1];
	}
	return ret;
}

long long next_interrupt_num() {
	return schedint[1].instno;
}

int interrupts_exist() {
	return sched_count>0;
}

void block_process (struct ctx_t* ctx) {
		pthread_mutex_lock(&ke->process_events_mutex);
		ctx->status = ctx_suspended;
		
		if (ctx->running_prev)
			ctx->running_prev->running_next = ctx->running_next;
		
		if (ctx->running_next)
			ctx->running_next->running_prev = ctx->running_prev;

		if(ke->running_list_head == ctx)
			ke->running_list_head = ctx->running_next;
		
		if(ke->running_list_tail == ctx)
			ke->running_list_tail = ctx->running_prev;
		
		ctx->suspended_prev = ke->suspended_list_tail;
		ctx->suspended_next = NULL;
		if (ke->suspended_list_tail)
			ke->suspended_list_tail->suspended_next = ctx;
		ke->suspended_list_tail = ctx;
		
		pthread_mutex_unlock(&ke->process_events_mutex);
}

void unblock_process (struct ctx_t* ctx) {
		pthread_mutex_lock(&ke->process_events_mutex);
		ctx->status = ctx_running;

		if (ctx->suspended_prev)
			ctx->suspended_prev->suspended_next = ctx->suspended_next;
		
		if (ctx->suspended_next)
			ctx->suspended_next->suspended_prev = ctx->suspended_prev;
		
		if(ke->suspended_list_head == ctx)
			ke->suspended_list_head = ctx->suspended_next;

		if(ke->suspended_list_tail == ctx)
			ke->suspended_list_tail = ctx->suspended_prev;

		ctx->running_prev = ke->running_list_tail;
		ctx->running_next = NULL;
		if (ke->running_list_tail)
			ke->running_list_tail->running_next = ctx;
		ke->running_list_tail = ctx;
